...
export default function NewProjectPage() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const optimizeResponse = await fetch(`/api/optimize/${data.id}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        },
      ...
      if (!optimizeResponse.ok) {
        ...
        throw { response: { status: optimizeResponse.status, data: errorData } }
      }

      toast.success('Optimizasyon tamamlandı!')
      router.push(`/projects/${data.id}/results`)
    } catch (error: any) {
