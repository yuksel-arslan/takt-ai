...
const EvolutionChart = ({ evolutionHistory }) => {
  if (!evolutionHistory?.best_fitness?.length) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <p className="text-gray-500 text-center">Evrim verisi bulunamadi</p>
      </div>
    )
  }
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {evolutionHistory.best_fitness.length} jenerasyon boyunca fitness degisimi
          </p>
        </div>
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 bg-blue-600 rounded-full"></div>
            <span className="text-xs text-gray-600">En Iyi Plan</span>
          </div>
          <div className="flex items-center space-x-2">
